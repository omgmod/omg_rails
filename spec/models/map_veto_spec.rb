...
RSpec.describe MapVeto, type: :model do
  ...
  describe "#competitive_veto_limit" do
    ...
    let(:competitive_map1) { create :map, name: "4p_Arras", category: "competitive" }
    let(:competitive_map2) { create :map, name: "4p_Langres", category: "competitive" }
    ...
    it "allows vetoing one competitive map" do
      veto = MapVeto.new(company: company, map: competitive_map1)
      expect(veto).to be_valid
    end

    it "prevents vetoing a second competitive map" do
      create :map_veto, company: company, map: competitive_map1
      veto = MapVeto.new(company: company, map: competitive_map2)
      expect(veto).not_to be_valid
      expect(veto.errors[:base]).to include("Company can only veto one competitive map")
    end
    ...
    it "allows vetoing a meme map alongside a competitive map" do
      create :map_veto, company: company, map: competitive_map1
